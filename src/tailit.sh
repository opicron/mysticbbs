...
for i in $(seq 1 $num_nodes); do
  ...
  tail -n0 -F "$log_file" | grep --line-buffered -Evf /mystic/grep_log_blacklist.txt | while IFS= read -r line; do
    ...
    curl -s -o /dev/null -X POST -H "Content-type: application/json" --data "{\"text\":\"${prefix}${escaped_line}\"}" https://hooks.slack.com/services/xxx/xxx
  done &
done
...
tail -n0 -F /mystic/logs/errors.log | grep --line-buffered -Evf /mystic/grep_log_blacklist.txt | while IFS= read -r line; do
    ...
    escaped_line=$(echo "$line" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g')
    curl -s -o /dev/null -X POST -H "Content-type: application/json" --data "{'text':\"${escaped_line}\"}" https://hooks.slack.com/services/xxx/xxx
done &
